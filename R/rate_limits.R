...
#' For Mistral, use [get_api_key_mistral()].
#'
...
.rate_limits_per_minute_engine <- function(
  ...
  if ("Mistral" %in% AI_tool && any(!is.element(model, mistral_model_prizes$model)))
    stop("Unknown model(s) used. Available models are: ",
         paste(mistral_model_prizes$model, collapse = ", "))
  ...
  body <- if ("Groq" %in% AI_tool | "Mistral" %in% AI_tool) groq_body else openai_body
  ...
  if (curl::has_internet()){
    ...
    if (status_code() == 200){
      ...
      if ("Mistral" %in% AI_tool) {
        rmp <- resp |>
          httr2::resp_header("x-ratelimit-limit-req-minute") |>
          as.numeric()

        tmp <- resp |>
          httr2::resp_header("x-ratelimit-limit-tokens-minute") |>
          as.numeric()
