...
screen_errors <- function(
  ...
  # Recover original args if present (used for backend inference when class is generic)
  arg_list <- attr(object, "arg_list")
  if (is.null(arg_list)) arg_list <- list()

  # Detect backend from explicit class and/or recorded api_url.
  backend_class <- intersect(class(object), c("gpt", "groq", "ollama"))
  api_url <- if (!is.null(arg_list$api_url) && is.character(arg_list$api_url) && length(arg_list$api_url) == 1) {
    tolower(arg_list$api_url)
  } else {
    ""
  }

  backend <- NULL
  if ("ollama" %in% backend_class) backend <- "ollama"
  if (is.null(backend) && "groq" %in% backend_class) backend <- "groq"

  if (is.null(backend) && nzchar(api_url)) {
    if (grepl("groq\\.com", api_url)) {
      backend <- "groq"
    } else if (grepl("localhost:11434|127\\.0\\.0\\.1:11434|/api/chat", api_url)) {
      backend <- "ollama"
    } else if (grepl("openai\\.com", api_url)) {
      backend <- "gpt"
    }
  }

  if (is.null(backend) && "gpt" %in% backend_class) backend <- "gpt"
  if (is.null(backend)) stop("Unknown object backend for screen_errors().")

  if (identical(backend, "ollama")) {
    stop("Rescreening not supported for Ollama-backed objects. Errors from tabscreen_ollama() are usually due to local resource constraints or invalid input.")
  }
  ...
  backend_fun <- switch(
    ...
    gpt = tabscreen_gpt,
    groq = tabscreen_groq,
    stop("No tabscreen function for this backend.")
  )

  error_data <- object$error_data
  if (is.null(error_data) || nrow(error_data) == 0) message("No errors to rescreen.")
